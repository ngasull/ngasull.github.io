...
- Nous utliserons **_conteneur_** pour se référer à ce qui est exécuté dans l'univers de Docker
...
_Exemple : les images `postgres` et `mysql` écrivent d'elles-mêmes une partie de leurs fichiers avec les les utilisateurs de PostgreSQL et MySQL._
...
<h5 no-toc>Configuration hôte impactée :</h5>
...
<h5 no-toc>/etc/docker/daemon.json</h5>
...
J'espère que cet article t'aura éclairé sur l'utilisation des _user namespaces_ pour gérer ses permissions Docker. Il s'agit d'un retour d'exprérience personnelle et de mes avis sur le sujet. N'hésite pas à explorer par toi-même, faire ta propre configuration et <a href="/fr/about/">me la partager</a> 🙂
